...
function CreatePost() {
    ...
    // const handleUploadImgClick=async()=>{
    //     if(!file){
    ...
    const handleCreatePostBtn=async(e)=>{
        e.preventDefault();
        try {
            setimageFileUploadError(null);
  ...
  return (
    ...
    <form onSubmit={handleCreatePostBtn} className="flex flex-col gap-4">
        <div className="flex flex-col md:flex-row gap-4 justify-between m-auto w-[90vw] sm:w-[80vw] md:w-[60vw] lg:w-[50vw]">
            <TextInput onChange={(e)=>setFormData(()=>({...formData,title:e.target.value}))} name='title' type='text' placeholder='Title *' required 
        ...
        <Button disabled={!!imageFileUploadProgress} type='submit' gradientDuoTone={"purpleToPink"} className='rounded-lg mb-12 p-1  m-auto w-[80vw] md:w-[60vw]'>Publish</Button>
